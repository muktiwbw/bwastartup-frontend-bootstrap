...
export default desiredContextRoot =>
  function projectHooksRouteRedirect(req, res, next) {
    const _parsedUrl = Reflect.has(req, '_parsedUrl') ? req._parsedUrl : null
    const url = _parsedUrl !== null ? _parsedUrl : parseurl(req)

    // The first time hitting the URL, it'll go here because the desired and the actual path name are different
    if (desiredContextRoot !== url.pathname) {
      const Location = desiredContextRoot
      
      // Set redirect header and redirect path
      res.writeHead(302, {
        Location
      })
    ...
    // After the redirect, the desired and actual path name are the same, thus skip the if statement
    next()
  }